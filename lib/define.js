...
module.exports = function(Qb, sql) {
	...
	Qb.prototype.define = function(definitions) {
		...
		for (var tableName in definitions) {
			...
			// "tableDef.name" should contain the actual name of the table
			// as it is used in the SQL whereas "tableName" is the key of
			// the table in the definitions. Usually these will be the same.

			var name = tableDef.name || tableName;

			// Register the model with sql.define.
			var sqlDef = { name: name, columns: sqlColumns };
...
function normalize(definitions) {
	...
	for (var tableName in definitions) {
		...
		tableDef.name = tableDef.name || tableName;
		...
		if (_.isArray(tableDef.joins)) {
			tableDef.joins.forEach(function(join) {
				...
				if (!definitions[join.name]) { 
					throw Error('Table ' + tableDef.name + ' joined on undefined table ' + join.name + '.'); 
				}
			...
			for (var joinName in tableDef.joins) {
				...
				if (!definitions[joinName]) { 
					throw Error('Table ' + tableDef.name + ' joined on undefined table ' + joinName + '.'); 
				}
		...
		// Filters (WHERE's) may be defined as an array of objects or
		// as a single object.

		// Singular to plural.
		if (tableDef.where) {
			tableDef.wheres = _.clone(tableDef.where);
			delete tableDef.where;
		}
		
		// If object, coerce to array.
		if (_.isObject(tableDef.wheres) && !_.isArray(tableDef.wheres)) {
			tableDef.wheres = [tableDef.wheres];
		}
