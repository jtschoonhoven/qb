...
Joins.prototype.toSQL = function(query, qb) {
	...
	// Each join in this.collection, call join.toSQL.
...
JoinSpec.prototype.toSQL = function(qb, joins, names, source) {
	...
	var joinDef  = qb.definitions[this.name];
	var joinName = joinDef.name || this.name;
	...
	this.model = qb.models[this.name].as(joinAlias);

	// Get keys for join. Default to primary key if source/target keys are not set.
	var sourceKey = sourceDef.joins[this.name].source_key || sourceDef.primary_key;
	var joinKey   = sourceDef.joins[this.name].target_key || joinDef.primary_key;
